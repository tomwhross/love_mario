...
function Player:init(map)
    ...
    self.behaviors = {
        ['idle'] = function(dt)
            ...
            elseif love.keyboard.isDown('left') and self:endGame() ~= true then
                self.direction = 'left'
                self.dx = -WALKING_SPEED
                self.state = 'walking'
                self.animations['walking']:restart()
                self.animation = self.animations['walking']
            elseif love.keyboard.isDown('right') and self:endGame() ~= true then
...
function Player:endGame()
    if self.x >= self.map.mapWidthPixels - 48 then
        ...
        self.x = self.map.mapWidthPixels - 48
